...
(function($) {
	...
	$.fn.FancyFileUpload = function(options) {
		...
		var UploadDone = function(e, data) {
			...
			{
				if (typeof(data.result.error) !== 'string')  data.result.error = Translate('The server indicated that the upload was not successful.  No additional pages available.');
		...
		var UploadChunkSend = function(e, data) {
			if (data.ff_info)
			{
				...
				if (data.ff_info.lastresult && !data.ff_info.lastresult.success)
				{
					...
					if (typeof(data.ff_info.lastresult.error) !== 'string')  data.ff_info.lastresult.error = Translate('The server indicated that the upload was not successful.  No additional pages available.');
		...
		return this.each(function() {
			...
			// Save necessary pages in case the uploader is destroyed later.
